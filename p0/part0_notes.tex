...
In R2, the Numba CUDA kernel (cell 5, lines 3--10 of the kernel function) does the same thing using Numba's helper:
...
The NumPy baseline in R2 (cell 3, lines 5--8 of \texttt{floyd\_warshall\_numpy}) partitions differently --- it operates on the entire matrix at once using vectorized broadcasting:
\begin{lstlisting}[language=Python]
for k in range(n):
...
In R2's Numba kernel (cell 5, lines 5--6 of \texttt{floyd\_step\_kernel}), each thread does the same:
\begin{lstlisting}[language=Python]
Dik = D[i, k]    # read from column k (global memory)
Dkj = D[k, j]    # read from row k (global memory)
\end{lstlisting}
...
In the NumPy version (R2, cell 3, \texttt{floyd\_warshall\_numpy}), communication is handled by extracting entire slices before broadcasting:
\begin{lstlisting}[language=Python]
Dik = D[:, k][:, None]     # entire column k as (n,1)
Dkj = D[k, :][None, :]     # entire row k as (1,n)
D = np.minimum(D, Dik + Dkj)
\end{lstlisting}
NumPy grabs the full row and column at once, then broadcasts the update across all cells internally.
...
In R2 (cell 5, lines 13--16 of \texttt{floyd\_warshall\_numba\_cuda}):
\begin{lstlisting}[language=Python]
threads = (16, 16)
...
In R1 (lines 90--92 of \texttt{floyd\_warshall\_cuda}):
\begin{lstlisting}[language=C++]
dim3 block(16, 16);
...
In R2 (cell 5, \texttt{floyd\_warshall\_numba\_cuda}):
\begin{lstlisting}[language=Python]
threads = (16, 16)
blocks = ((n + threads[0] - 1) // threads[0],
          (n + threads[1] - 1) // threads[1])
...
\begin{enumerate}
  \item \textbf{(R1/R2 -- Block Size Choice)} Both R1 and R2 use a block size of $16 \times 16$. Why was $16 \times 16$ chosen instead of $32 \times 32$ or $8 \times 8$? How does block size affect GPU occupancy, register usage, and overall kernel performance?
...
\end{lstlisting}
  ...
  \item \textbf{(R1/R2 -- No Shared-Memory Tiling)} Neither R1 nor R2 uses shared memory in their GPU kernels. Every thread independently reads \texttt{D[i][k]} and \texttt{D[k][j]} from global memory.

  In R1 (lines 75--76 of \texttt{floyd\_step\_kernel}):
\begin{lstlisting}[language=C++]
float Dik = D[ik];   // D[i][k] from global memory
float Dkj = D[kj];   // D[k][j] from global memory
\end{lstlisting}

  In R2 (cell 5, lines 5--6 of \texttt{floyd\_step\_kernel}):
\begin{lstlisting}[language=Python]
Dik = D[i, k]    # D[i][k] from global memory
Dkj = D[k, j]    # D[k][j] from global memory
\end{lstlisting}

  Within a single $16 \times 16$ block, all 16 threads in the same row read the same \texttt{D[i][k]} value, and all 16 threads in the same column read the same \texttt{D[k][j]} value. That means 256 threads produce up to 512 global memory reads, when only 32 unique values are actually needed (16 from row $k$ and 16 from column $k$). Loading these into shared memory once and letting all threads read from there would eliminate this redundancy. Why was this optimization left out, and how much improvement could it provide?

  \item \textbf{(R1/R2 -- One Kernel Launch Per $k$)} In both R1 and R2, the host launches a separate kernel for every value of $k$, resulting in $n$ kernel launches for an $n$-node graph. Each launch carries overhead: the GPU must set up the grid, dispatch threads, and synchronize before the next launch.

  In R1 (lines 94--96 of \texttt{floyd\_warshall\_cuda}):
\begin{lstlisting}[language=C++]
for (int k = 0; k < n; ++k) {
...
\end{lstlisting}
  ...
  For large $n$ (say 2048), that is 2048 kernel launches, each with a small but non-zero cost. Could multiple $k$-steps be batched or fused into a single kernel (where safe) to reduce this repeated launch and synchronization overhead? What constraints does the data dependency on $k$ impose on such batching?

  \item \textbf{(R1/R2 -- No Hierarchical Decomposition)} Both R1 and R2 use a flat, single-level decomposition: the entire $n \times n$ matrix is covered by one 2D grid of $16 \times 16$ thread blocks. The full kernel launch in R1 (lines 90--96) shows this flat structure:
\begin{lstlisting}[language=C++]
dim3 block(16, 16);
dim3 grid((n + block.x - 1) / block.x,
          (n + block.y - 1) / block.y);
